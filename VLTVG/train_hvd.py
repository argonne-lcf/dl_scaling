...
from engine import train_one_epoch, train_one_epoch_ddp, train_one_epoch_hvd, train_one_epoch_w_accum, evaluate, evaluate_ddp
...
def get_args_parser():
    ...
    # dataset parameters
    parser.add_argument('--data_root', default='/grand/datascience/zhaozhenghao/datasets/VG/data/')
    parser.add_argument('--split_root', default='/grand/datascience/zhaozhenghao/datasets/VG/split/')
...
def main(args):
    ...
    optimizer = torch.optim.AdamW(param_dicts, lr=args.lr * hvd.size(),
    ...
    if args.distributed:
        ...
        sampler_val = DistributedSampler(dataset_val, num_replicas=hvd.size(), rank=hvd.rank(), shuffle=False)
    else:
    ...
    data_loader_val = DataLoader(dataset_val, sampler=sampler_val, batch_size=args.batch_size_val, 
                                pin_memory=True, shuffle=False, num_workers=0)

    epoch_trainer = train_one_epoch_hvd
    if args.enable_batch_accum:
        epoch_trainer = train_one_epoch_w_accum
    epoch_eval = evaluate_ddp # for evaluation horovod is nothing different from ddp
    ...
    start_time = time.time() # to calculate total training time
    # end_time = time.time() # iterate over epochs
    # best_acc = 0
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = epoch_trainer(
            hvd.rank(), model, criterion, data_loader_train, optimizer, device, epoch, args.epochs, args.clip_max_norm
        )
        ...
        # )
        # val_time = time.time() - end_time - training_time
        ...
        if args.output_dir and utils.is_main_process() and hvd.rank() == 0:
            ...
            #      f'training: {training_time:.4f}',
            #      f'val: {val_time:.4f}',
            #      f'total: {training_time + val_time:.4f}']))
            epoch_logger.info('\n')
    ...
    if hvd.rank() == 0:
        ...
        logger.info('Throughput: {:.4f} samples/s'.format(len(data_loader_train.dataset) * args.epochs / total_time))
