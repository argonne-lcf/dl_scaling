...
from engine import train_one_epoch, train_one_epoch_ddp, train_one_epoch_w_accum, evaluate, evaluate_ddp
...
def main(args):
    ...
    if args.distributed:
        sampler_train = DistributedSampler(dataset_train, num_replicas=size, rank=rank)
        sampler_val = DistributedSampler(dataset_val, num_replicas=size, rank=rank, shuffle=False)
    else:
    ...
    epoch_trainer = train_one_epoch_ddp
    ...
    logger.info('Throughput: {:.2f} samples/s'.format(len(data_loader_train.dataset) * args.epochs / total_time))
