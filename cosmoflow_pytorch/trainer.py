        ...
        with utils.ProfilerSection("training step", self._enable_profiling):
            if hasattr(self.model, "graph_capture"):
                ...
                if self._amp:
                    ...
                    self.scaler_.step(self.optimizer)
                    self.scaler_.update()
                    self.optimizer.synchronize() # horovod
                else:
                    y = self.model(x)
                    loss = self.loss_fn(y, y_hat)
                    loss.backward()
    ...
    def train_epoch(self,
        ...
        with utils.ProfilerSection(f"training epoch #{epoch}",
            ...
            try:
                ...
                should_run = False
            t1 = time()
            perftrace.event_complete('train_epoch', 'prefetch', t0, t1 - t0)                            
            while should_run:
                ...
                with utils.ProfilerSection("convert", self._enable_profiling):
                    ...
                    label = input_data[1]
                t1 = time()
                perftrace.event_complete('train_epoch', 'convert', t0, t1 - t0)
                t0 = time()
                try:
                    with torch.cuda.stream(self.prefetch_stream):
                        input_data = next(train_iter)
                ...
                elif distribute == "horovod":
                    self.train_step_hvd(data, label)
#                perftrace.event_complete('train_epoch', 'sleep', t0, t1 - t0)                
                if ("profile_range" in self._config and
    ...
    def eval_epoch(self,
        ...
        if distribute == "ddp":
            ...
            return self.score_fn.get_value_hvd(distributed=not self._distenv.is_single,
                                        pg_handler=None)
    @perftrace.event_logging
    def epoch_step(self,
                   train_iter: DataIter,
        ...
        with utils.CudaExecutionTimer() as train_latency:
            if not eval_only:
                num_steps = self.train_epoch(train_iter, epoch, distribute)

        utils.logger.start(key=utils.logger.constants.EVAL_START,
                           metadata={'epoch_num': epoch + 1})
        with utils.CudaExecutionTimer() as eval_latency:
            validation_score = self.eval_epoch(eval_iter, epoch, distribute)
        ...
        #                              'step': epoch})
        utils.logger.event(key='tracked_stats',
                           value={"throughput": self._config["data"]["batch_size"] * self._config["data"]["num_nodes"] * 4 * num_steps /
                                  (train_epoch_elapsed / 1000)},
                           metadata={'epoch_num': epoch+1,
                                     'step': epoch})
        utils.logger.event(key='eval_error',
                               ...
                               metadata={'epoch_num': epoch + 1})
        utils.logger.event(key='epoch_time',
                           value={"training_time": train_epoch_elapsed,
                                  "eval_time": eval_epoch_elapsed},
                            metadata={'epoch_num': epoch + 1})
    ...
    def warmup_hvd(self, capture_stream: Optional[torch.cuda.Stream] = None):
        if self._amp:
            begin_scale = self.scaler_.get_scale()
            ...
            with torch.cuda.stream(capture_stream):
                ...
                for _ in range(50):                    
                    ...
                    if self._amp:
                        ...
                        loss = self.loss_fn(y, static_input_label)
                        loss.backward()       
                    self.optimizer.synchronize() # adjust for horovod          

            torch.cuda.current_stream().wait_stream(capture_stream)

            if self._config["model"]["cuda_graph"] == True: # horovod can not use cuda graph, so set it to false in config
                self.optimizer.zero_grad()
                self.model.graph_capture = torch.cuda.CUDAGraph()
                self.model.zero_capture = torch.cuda.CUDAGraph()
                self.model.static_input_data = static_input_data
        ...
        for _ in range(10):
            self.optimizer.step()
            hvd.allreduce(static_input_data.contiguous()) # horovod
