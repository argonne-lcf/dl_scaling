...
from engine import train_one_epoch, train_one_epoch_ddp, train_one_epoch_deepspeed, train_one_epoch_w_accum, evaluate, evaluate_ddp
...
def get_args_parser():
    ...
    parser.add_argument('--polaris_nodes', default=1, type=int)
...
def main(args):
    ...
    data_loader_val = DataLoader(dataset_val, batch_size=args.batch_size_val,
                                shuffle=False, num_workers=0)

    epoch_trainer = train_one_epoch_deepspeed
    if args.enable_batch_accum:
        epoch_trainer = train_one_epoch_w_accum
    epoch_eval = evaluate_ddp

    output_dir = Path(args.output_dir)
    # do not handle the case of resume
    # if args.resume:
    ...
    model_engine, optimizer, _, __ = deepspeed.initialize(args=args, model=model, model_parameters=parameters)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = epoch_trainer(
            args.steps, model_engine, criterion, data_loader_train, optimizer, device, epoch, args.epochs, args.clip_max_norm,
        )
    ...
    # precise_time = total_time - (time.time() - eval_time)*args.epochs + (time.time() - eval_time)*args.epochs/(4*args.polaris_nodes)
    total_time_str = str(datetime.timedelta(seconds=int(total_time)))
    logger.info('Training time {}'.format(total_time_str))
    # precise_time_str = str(datetime.timedelta(seconds=int(precise_time)))
    # logger.info('Precise training time {}'.format(precise_time_str))
    logger.info('Throughput: {:.2f} samples/s'.format(args.steps * args.batch_size * 4 * args.polaris_nodes / total_time))
