...
class data_prefetcher():
    ...
    def preload(self):
        ...
        with torch.cuda.stream(self.stream):
            ...
            #     print(k, tensor_dict[k].shape, type(tensor_dict[k]))
            # breakpoint()
...
def train_one_epoch_ddp(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for iteration, (img, mask, target) in enumerate(data_loader):
        img = img.to(device, non_blocking=True)
        mask = mask.to(device, non_blocking=True)

        target = utils.TensorDict({k: v for k, v in target.items() if k != 'phrase'})

        target_dict = target.tensor_dict
        target_dict = {k: target_dict[k].to(device, non_blocking=True) for k in target_dict}
        word_id, word_mask = target_dict['word_id'], target_dict['word_mask']
...
def train_one_epoch_hvd(rank, model: torch.nn.Module, criterion: torch.nn.Module,
                    data_loader, optimizer: torch.optim.Optimizer,
                    device: torch.device, epoch: int, epochs: int, max_norm: float = 0):
    model.train()
    criterion.train()
...
def train_one_epoch_deepspeed(steps, model_engine, criterion: torch.nn.Module,
                    data_loader, optimizer: torch.optim.Optimizer,
                    device: torch.device, epoch: int, epochs: int, max_norm: float = 0):
    ...
    for iteration, (img, mask, target) in enumerate(data_loader):
        img = img.to(model_engine.local_rank)
        mask = mask.to(model_engine.local_rank)

        target = utils.TensorDict({k: v for k, v in target.items() if k != 'phrase'})

        target_dict = target.tensor_dict
        target_dict = {k: target_dict[k].to(model_engine.local_rank) for k in target_dict}
        word_id, word_mask = target_dict['word_id'], target_dict['word_mask']

        data_time.update(time.time() - end)

        outputs = model_engine(img, mask, word_id, word_mask)
...
@torch.no_grad()
def evaluate_ddp(model, criterion, postprocessor, data_loader, device, save_path=''):
    model.eval()
    if criterion:
        criterion.eval()
